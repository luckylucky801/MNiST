...
def clustering(adata, n_clusters=7, radius=50, key='emb', method='mclust', start=0.1, end=3.0, increment=0.01, refinement=False):
    ...
    if refinement:  
       ...
       new_type = pd.Categorical(new_type)
...
def refine_label(adata, radius=50, key='label'):
    ...
    for i in range(len(old_type)):
        ...
        neigh_type = old_type[indices[i, :]]
        neigh_type_count = pd.Series(neigh_type).value_counts()

        if neigh_type_count.loc[old_type[i]] < n_neigh / 2 and neigh_type_count.max() > n_neigh / 2:
            new_type.append(neigh_type_count.idxmax()) 
        else:
